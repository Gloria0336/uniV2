      ...
      <div className="relative w-full max-w-6xl h-full flex flex-col bg-void-black border border-neon-blue/40 shadow-[0_0_60px_rgba(0,243,255,0.1)] overflow-hidden">
        ...
        <div className="flex border-b border-neon-blue/30 bg-black/80">
          <div className="px-6 py-4 border-r border-neon-blue/20 flex items-center">
            <h2 className="text-xl font-bold text-neon-blue tracking-tighter uppercase italic">太陽系時報 (Solar Times)</h2>
          </div>
          ...
          {activeTab === 'OFFICIAL' && (
            <div className="p-6 md:p-10 max-w-5xl mx-auto font-serif">
                ...
                <div className="text-center border-b-4 border-white pb-6 mb-8">
                    <div className="text-[10px] tracking-[0.5em] text-gray-500 uppercase mb-2 font-mono">地球聯合政府授權網絡 (EUG Authorized Network)</div>
                    ...
                    <div className="flex justify-between items-center mt-4 text-[10px] font-mono text-gray-400 px-2 uppercase italic tracking-widest">
                        <span>版本: 3150-Q1</span>
                        <span>售價: 5.00 CR</span>
                        <span>驗證碼: EUG-77-DELTA</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    {safeNews.length === 0 ? (
                        <div className="col-span-2 text-center py-20 opacity-20 text-4xl italic font-mono uppercase">等待訊號傳輸...</div>
                    ) : (
                        safeNews.map((item, idx) => (
                            <div key={idx} className={`space-y-3 pb-8 ${idx < safeNews.length - 1 ? 'border-b border-white/20' : ''}`}>
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[9px] text-gray-500 font-mono italic">[ 已驗證 ]</span>
                                </div>
                                ...
                                <p className="text-sm text-gray-400 leading-relaxed indent-4">
                                    {item.status}. 根據最近的星際情報顯示，該地區的局勢正在發生深刻變化。官員表示將持續關注動態並確保航道安全。
                                </p>
                            </div>
                        ))
                    )}
          ...
          {activeTab === 'GOSSIP' && (
            <div className="p-6 md:p-10 font-mono">
                <div className="flex items-center gap-3 mb-8 bg-pink-900/10 border border-pink-500/30 p-4 rounded animate-pulse">
                    ...
                    <div>
                        <div className="text-pink-400 font-bold uppercase tracking-widest">連線狀態：無法追蹤</div>
                        <div className="text-[10px] text-pink-600">警告：偵測到神經脈衝。資訊可靠性不予保證。</div>
                    </div>
                </div>

                <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
                    {safeGossip.length === 0 ? (
                        <div className="text-pink-900 text-center py-20 text-4xl uppercase animate-pulse">沉默即是死亡...</div>
                    ) : (
                        safeGossip.map((item, idx) => (
                            <div key={idx} className="break-inside-avoid bg-black border-l-4 border-pink-600 p-4 shadow-[5px_5px_0_rgba(219,39,119,0.2)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all">
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    <span className="text-[8px] text-white/40 uppercase tracking-tighter">代碼：#{Math.floor(Math.random()*90000)}</span>
                                </div>
                                ...
                                <div className="flex justify-between items-center text-[9px]">
                                    <div className="flex gap-2">
                                        <span className="text-pink-600 font-bold">#外流</span>
                                        <span className="text-pink-600 font-bold">#流言</span>
                                    </div>
          ...
          {activeTab === 'HISTORY' && (
            <div className="p-6 md:p-10 font-mono max-w-4xl mx-auto">
                <div className="mb-10 text-center">
                    <h3 className="text-2xl font-bold text-yellow-500 tracking-widest uppercase mb-2">太陽系編年史</h3>
                    <div className="text-[10px] text-gray-500">唯讀資料庫 // 最後更新：第 {safeNews.length > 0 ? safeNews[0].turn : '??'} 回合</div>
                </div>

                <div className="relative border-l-2 border-yellow-500/20 ml-4 space-y-12 pb-10">
                    {safeChronicles.length === 0 ? (
                        <div className="text-yellow-900 text-center py-20 text-2xl uppercase">歷史尚未被書寫...</div>
                    ) : (
        ...
        <div className="p-2 border-t border-neon-blue/20 bg-black flex justify-between items-center px-6">
            <div className="text-[8px] text-gray-700 font-mono uppercase tracking-[1em]">正在掃描訊號... 訊號強度 100%</div>
